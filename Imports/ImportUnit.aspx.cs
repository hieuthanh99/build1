...
public partial class Imports_ImportUnit : BasePage
{
    ...
    const string KEY = "8b3a3d17-5a53-45cf-bc6b-cbf3dbdfd3df";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Session[KEY] = null;
                FromMonthEditor.Value = DateTime.Now.Month - 1;
                ToMonthEditor.Value = DateTime.Now.Month - 1;
            }
    ...
    {
        var list = entities.ETParameters.Where(x => x.ETCode == "UNIT").OrderBy(x => x.CreateDate).ToList();
    ...
    protected void FileHistoryGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
    {
        ...
        if (args[0] == "Apply")
        {
            try
            {
                ...
                int fromMonth = Convert.ToInt32(FromMonthEditor.Value);
                int toMonth = Convert.ToInt32(ToMonthEditor.Value);

                var entity = entities.ETHistories.SingleOrDefault(x => x.HistoryID == historyKey);

                entities.TransferUnitData(historyKey, versionKey, fromMonth, toMonth, SessionUser.UserID);
