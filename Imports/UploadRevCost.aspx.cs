...
public partial class Imports_UploadRevCost : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                string fileType= this.cboFileType.Value.ToString();
    ...
    protected void cboAreaCode_Init(object sender, EventArgs e)
    {
        ...
        {
            var list = entities.Airports.Where(x => x.Code == aReaCode).ToList();
            s.DataSource = list;
        }
    ...
    {
        var list = entities.ETParameters.Where(x => x.ETCode == "ULREVCOST").OrderBy(x => x.ETCode).ToList();
    ...
    private ETHistory InsertHistory(string fileName, string fileType, string areaCode)
    {
    ...
    protected void FileHistoryGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
    {
        ...
        if (args[0] == "ProcessFile")
        {
            ...
            var history = InsertHistory(fileName, fileType, areaCode);
            ...
            // Update thong tin companyName, Subaccount
            //entities.UpdateInfoUploadRevCost(history.HistoryID);
        ...
        if (args[0] == "ApplyToVersion")
        {
            ...
            if (!decimal.TryParse(args[2], out hisID))
                return;
            if (!int.TryParse(args[3], out FromMonth))
                return;
            if (!int.TryParse(args[4], out ToMonth))
                return;
            

            var entity = entities.ETHistories.SingleOrDefault(x => x.HistoryID == hisID);

            //entities.TransferUploadCostRevData(versionID, hisID,FromMonth,ToMonth, SessionUser.UserID);

           // LoadHistories(entity.ETCode,entity.AreaCode);
    ...
    protected void btnUpdateInfoFile_Click(object sender, EventArgs e)
    {
       // entities.UpdateInfoUploadRevCost()
    }
    ...
    protected void cboPlanMonth_Init(object sender, EventArgs e)
    {
        ASPxComboBox s = sender as ASPxComboBox;
        ListEditItem le = new ListEditItem();
        for (int i = 1; i <= 12; i++)
    ...
    protected void cboPlanVersion_Init(object sender, EventArgs e)
    {
        ASPxComboBox s = sender as ASPxComboBox;
        var list = entities.Versions.Where(x => x.Active ?? true).OrderByDescending(x => x.VersionYear).ThenBy(x => x.VersionType).ThenByDescending(x => x.VersionID).ToList();
        ListEditItem le = new ListEditItem();
