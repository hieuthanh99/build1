...
public partial class Reports_FASTReport1 : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            this.seMonth.Value = DateTime.Now.Month;
    ...
    protected MemoryStream CreateXlsxFileReport()
    {
        ...
        if (rdReport.Value.ToString() == "BCCTSXKD")
        {
            report = new FastReport1();
        }
        else if (rdReport.Value.ToString() == "KQSXKD")
        {
            report = new FastReport2();
            report.Parameters["pTemplate"].Value = cboTemplate.Value;
        }

        int month = Convert.ToInt32(this.seMonth.Value);
        int year = Convert.ToInt32(this.seYear.Value);
        report.Parameters["pMonth"].Value = month;
